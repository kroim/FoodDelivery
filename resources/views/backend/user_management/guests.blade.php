    ...
    <section class="content">
        <div class="content__inner">
            <header class="content__title">
                <h1>{{__('global.side.guests')}}</h1>
            </header>

            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">{{__('global.side.user').' '.__('global.common.management')}}</h4>
                    @if(Auth::user()->role < 3 || in_array('user_add', $user_permission))
                        <div class="actions">
                            <button class="btn btn-success" href="javascript:" onclick="$('#modal_add_user').modal('show')">{{ __('global.common.add').' '.__('global.side.user') }}</button>
                        </div>
                    @endif
